...
expr: INTEGER_NUMBER                                                                                    { $$=create_const_integer_expr(Integer, $1); }
    ...
    | SUPER OPEN_ROUND_BRACKET new_lines_op expr_list CLOSE_ROUND_BRACKET                               { $$=create_super_call_expr($4); }
    | VAR_OR_METHOD_NAME                                                                                { $$=create_const_string_expr(var_or_method, $1); }     
    | INSTANCE_VAR_NAME                                                                                 { $$=create_const_string_expr(instance_var, $1); }
    | CLASS_NAME DOT_SYMBOL new_lines_op NEW                                                            { $$=create_const_string_expr(class_name, $1); }
...
class_declaration: CLASS CLASS_NAME stmt_ends def_method_list_op END stmt_ends                      { $$=create_class_declaration_program_item($2, 0, $4); }
    | CLASS CLASS_NAME LESS_OP new_lines_op CLASS_NAME stmt_ends def_method_list_op END stmt_ends   { $$=create_class_declaration_program_item($2, $5, $7);  }
    ;
